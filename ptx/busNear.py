...
def intoDB(data):
    ...
    table = "\
        REPLACE INTO sb_realtime.BusNearStop(\
            plateNum, routeId, direction, stopid, busStatus, event, recordtime\
        ) VALUES "
    ...
    for i in range(len(data)):
        try:
            sql += ["( '%s', '%s', '%s', '%s', '%s', '%s', '%s' )"%(
                ...
                -1 if "A2EventType" not in data[i] else data[i]["A2EventType"],
...
def main():
    if sys.argv[1] == "TPE":
        url = "http://ptx.transportdata.tw/MOTC/v2/Bus/RealTimeNearStop/City/Taipei?%24format=JSON"
    else :
        url = "http://ptx.transportdata.tw/MOTC/v2/Bus/RealTimeNearStop/City/NewTaipei?%24format=JSON"
