...
def wait_midnight():
    while True:
        ...
        sys.stdout.flush()
        if (int(hours) == 2 and int(mins) == 0 and int(sec) == 1):
            break
        time.sleep(1)
class process:
...
#        wait_midnight()
        thlen = 10
        datehour = "16_12_11_09"
        startt = datetime.datetime.today()
        #job = process(datehour, "0400081000")
        #job.do("0", verbose = True)
        #quit()
        que = queue.Queue()  
...
#        print cursor._executed
        quit()
        d = datetime.datetime.today().replace(hour=9)
        date = d.strftime("%y_%m_%d")
 
        for i in range(-26,-2):
            if i in (-24,-23,-22):# Skip hous : 2(-24), 3(-23), 4(-22) Start At 0(-26), End To 23(-3)
                continue
            k = d + datetime.timedelta(hours=i)
            date = k.strftime("%y_%m_%d")
            hour = k.strftime("%H")
