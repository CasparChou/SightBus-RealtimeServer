...
def parserData(data, logtime):
    ...
    for i in range(len(data)): 
        if len(data) < 2:
            print "Data Fetch Nothing!"
            break
        try:
            srcTime = data[i]["UpdateTime"]
            if "SrcUpdateTime" in data[i]:
                srcTime = data[i]["SrcUpdateTime"]
            if "TransTime" in data[i]:
                srcTime = data[i]["TransTime"]
    	    ...
    	    %(
                ...
                data[i]["Direction"],
                srcTime.replace('T', ' ').replace('+08:00', '')
            )]
        except KeyError,e :
            print "KeyError: "
            print e
